...
export default function QuantAudit({ isMobile = false }) {
  ...
  async function runAudit() {
    setStatus('loading');
    setResults([]);
    // Animated delay — gives the UI time to show the scanning animation
  ...
  return (
    <div style={{ fontFamily: F, background: '#030608', borderTop: '1px solid #0d1117', width: '100%', boxSizing: 'border-box' }}>
      ...
      {/* Run button */}
      <div style={{ padding: pad, borderBottom: status === 'loading' ? 'none' : '1px solid #0d1117' }}>
        <button onClick={runAudit} disabled={status === 'loading'} style={{
          ...
          {status === 'loading' ? '⏳ Scanning Options Chain…' : '▶ Run Quant Audit'}
        </button>
        {status === 'error' && (
      ...
      {status === 'loading' && (
        <div style={{ padding: pad, borderBottom: '1px solid #0d1117' }}>
          ...
          `}</style>
          <div style={{ animation: 'scanFade 0.3s ease' }}>
            <div style={{ fontSize: '9px', color: '#00aaff', letterSpacing: '2.5px', textTransform: 'uppercase', marginBottom: '14px', fontWeight: 700 }}>
              ⚡ Scanning Live Options Chain
            </div>
            {[
              'Connecting to CBOE data feed…',
              'Fetching bid/ask spreads for TSLA…',
              'Filtering 20–45 DTE contracts…',
              'Calculating midpoint premiums…',
              'Scoring strategies by AROC…',
            ].map((line, i) => (
              <div key={i} style={{
                display: 'flex', alignItems: 'center', gap: '10px',
                marginBottom: '8px',
                animation: `scanFade 0.4s ease ${i * 0.35}s both`,
              }}>
                <div style={{
                  width: '100%', maxWidth: '220px', height: '3px',
                  background: 'linear-gradient(to right, #00aaff, #00ff88)',
                  borderRadius: '2px', transformOrigin: 'left',
                  animation: `scanPulse 1.4s ease-in-out ${i * 0.2}s infinite`,
                }} />
                <span style={{ fontSize: '10px', color: '#444', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>{line}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Results */}
      {status === 'done' && (
        <div style={{ padding: pad, boxSizing: 'border-box', width: '100%', maxWidth: '100%', overflow: 'hidden', display: 'block' }}>
          {results.length > 0 ? (
            <>
                ...
                {results.map((r, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, minmax(0, 1fr))', gap: '8px', paddingTop: '12px', borderTop: '1px solid #0d1117', marginBottom: '14px', width: '100%', boxSizing: 'border-box' }}>
                        <div>
