...
export default function ProgressiveGraph({ catalysts, links, onNodeClick, expandAll, onAllExpanded, isMobile = false, highlightedIds = null, highlightedCategories = null, smartMode = false, smartBadge = null, smartExpandCats = null, smartExpandTrigger = 0, onExitSmart = null }) {
  ...
  const rebuild = useCallback((expandedSet, prevMap) => {
    ...
    simRef.current = d3.forceSimulation(nodesCopy)
      ...
      .force('boundX',    d3.forceX(mob ? (W + 50) / 2 : W / 2).strength(0.06))
      .force('boundY',    d3.forceY(mob ? (H - 170) / 2 : (H - 60) / 2).strength(0.08))
      .alpha(0.4)
      .alphaDecay(0.001)
      .velocityDecay(0.88);
  }, [catalysts, links]);
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      nodesRef.current.forEach(node => {
        ...
        // Mobile: keep clear of Breaking News tab (left ~38px) and Ask Roger FAB + footer (bottom ~115px)
        const xMin = isMobile ? r + 50 : pad;
        const xMax = W - pad;
        const yMin = pad;
        const yMax = isMobile ? H - r - 170 : H - pad - 60;
        if (node.x !== undefined) node.x = Math.max(xMin, Math.min(xMax, node.x));
        if (node.y !== undefined) node.y = Math.max(yMin, Math.min(yMax, node.y));
      });
  ...
  useEffect(() => {
    function onResize() {
      ...
      if (simRef.current) {
        simRef.current
          .force('center', d3.forceCenter(isMobile ? (cssW+50)/2 : cssW/2, isMobile ? (cssH-170)/2 : cssH/2).strength(0.08))
          .force('boundX', d3.forceX(isMobile ? (cssW+50)/2 : cssW/2).strength(0.08))
          .force('boundY', d3.forceY(isMobile ? (cssH-170)/2 : (cssH-60)/2).strength(0.08))
          .alpha(0.5).restart();
      }
    }
