...
const FALLBACK = {
  ...
  delta: -0.26,
  conviction: "STANDARD",
  quant_note: "The $350 CSP May 22 offers the highest annualized return at 28.38% AROC with 28 DTE — near-peak Theta decay territory. At -0.26 delta, the strike sits ~5.9% OTM from ~$372, providing a meaningful buffer. Yield/risk tradeoff leads all CSP candidates by over 150bps annualized.",
  updated: "Apr 23, 2026 · 11pm ET",
};

export default function ThetaGangSignal({ isMobile = false }) {
  const { data: signal, loading } = useRemoteData('theta-gang.json', FALLBACK);
  ...
  const premiumFormatted = signal.premium
    ? `$${Number(signal.premium).toFixed(2)}`
    : '—';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '10px', letterSpacing: '2.5px', textTransform: 'uppercase', color: '#f59e0b', fontWeight: 800 }}>
            ⚡ Theta Gang Signal
          </span>
          <span style={{
            fontSize: '8px', letterSpacing: '1.5px', textTransform: 'uppercase',
            color: convictionColor,
            border: `1px solid ${convictionBorder}`,
            padding: '1px 6px',
      ...
      <div style={{
        ...
        <div style={{ fontSize: '9px', color: '#f59e0b', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '5px', fontWeight: 700 }}>
          Quant Note
        </div>
        <div style={{
